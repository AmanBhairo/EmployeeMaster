...
namespace CivicaEmployeeMaster.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        [HttpGet("GetTotalSalaryByYearByStoreProcedure")]
        public IActionResult GetTotalSalaryByYearByStoreProcedure(int year)
        {
            var response = _employeeService.GetTotalSalaryByYearByStoreProcedure(year);
            if (!response.Success)
            {
                return NotFound(response);
            }
        ...
        [HttpGet("GetTotalSalaryByMonthByStoreProcedure")]
        public IActionResult GetTotalSalaryByMonthByStoreProcedure(int month, int year)
        {
            var response = _employeeService.GetTotalSalaryByMonthByStoreProcedure(month, year);
            if (!response.Success)
            {
                return NotFound(response);
            }
        ...
        [HttpGet("GetTotalSalaryByMonthYearAndIdByStoreProcedure/{employeeId},{month},{year}")]
        public IActionResult GetTotalSalaryByMonthYearAndIdByStoreProcedure(int employeeId, int month, int year)
        {
            var response = _employeeService.GetTotalSalaryByMonthYearAndIdByStoreProcedure(employeeId, month, year);
            if (!response.Success)
            {
                return NotFound(response);
            }
        ...
        [HttpGet("GetTotalSalaryByYearAndIdByStoreProcedure/{employeeId},{year}")]
        public IActionResult GetTotalSalaryByYearAndIdByStoreProcedure(int employeeId, int year)
        {
            var response = _employeeService.GetTotalSalaryByYearAndIdByStoreProcedure(employeeId, year);
            if (!response.Success)
            {
                return NotFound(response);
            }
